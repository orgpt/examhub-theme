...
$blog_title       = $posts_page_id ? get_the_title( $posts_page_id ) : __( 'مدونة امتحانكوم', 'examhub' );
          ...
          <nav class="eh-blog-pagination mt-4 mt-lg-5" aria-label="<?php esc_attr_e( 'التنقل بين المقالات', 'examhub' ); ?>">
            ...
            the_posts_pagination(
                [
                    ...
                    'screen_reader_text' => __( 'التنقل بين المقالات', 'examhub' ),
                ]
            );
            ?>
          </nav>
        ...
        <div class="eh-blog-sidebar">
          <section class="eh-blog-sidebar-card">
            ...
            <form role="search" method="get" class="eh-blog-search" action="<?php echo esc_url( home_url( '/' ) ); ?>">
              <label class="screen-reader-text" for="eh-blog-search"><?php esc_html_e( 'ابحث عن:', 'examhub' ); ?></label>
              <input id="eh-blog-search" type="search" class="form-control" placeholder="<?php esc_attr_e( 'ابحث عن مقالات أو مواضيع أو نصائح...', 'examhub' ); ?>" value="<?php echo esc_attr( get_search_query() ); ?>" name="s">
